...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: DataFrame):
        ...
        try:
            ...
            print(f"Expected columns: {(self._schema_config['columns'])}, Actual columns: {(dataframe.columns)}")
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            # Checking col len (test/trin df)
            status = self.validate_number_of_columns(dataframe=train_df)
            if not status:
                validation_error_msg += f"Columns are missing in train df. "
            else:
            ...
            if not status:
                ...
                logging.info(f"All required columns present in test df")
            ...
            if not status:
                ...
                logging.info(f"All categorical/int columns present in train df: {status}")

            status = self.is_column_exist(df=test_df)
            if not status:
                validation_error_msg += f"Columns are missing in test df."
            else:
                logging.info(f"All categorical/int columns present in test df: {status}")
